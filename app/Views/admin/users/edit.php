...
<div class="card shadow-sm">
    <div class="card-body">
        <form action="<?= site_url($locale . '/admin/users/' . (isset($user['id']) ? 'edit/' . $user['id'] : 'create')) ?>" method="post">
            <?= csrf_field() ?>
            ...
            <div class="row mb-3">
                <div class="col-md-4">
                    <label for="password" class="form-label">
                        Password <?= !isset($user['id']) ? '<span class="text-danger">*</span>' : '' ?>
                    </label>
                    <input type="password" name="password" id="password" class="form-control"
                        <?= !isset($user['id']) ? 'required' : '' ?>
                           placeholder="<?= isset($user['id']) ? 'Leave empty to keep current' : 'Enter password' ?>">
                </div>
                <div class="col-md-4">
                    ...
                    <input type="password" name="pass_confirm" id="pass_confirm" class="form-control"
                           placeholder="<?= isset($user['id']) ? 'Leave empty to keep current' : 'Confirm password' ?>">
                </div>
                <div class="col-md-2">
            ...
            <div class="d-flex justify-content-between">
                ...
                <button type="submit" class="btn btn-primary">
                    <i class="bi bi-check-circle"></i> <?= isset($user['id']) ? 'Update' : 'Create' ?> User
                </button>
            </div>
        </form>
    </div>
